...
export const getStudentProfile = async (req: AuthenticatedRequest, res: Response) => {
  ...
  // If a username is provided in params, check if the requester is an admin/staff
  const targetUsername = (req.params.username || user.username).toUpperCase();
  
  if (req.params.username && user.role === UserRole.STUDENT && user.username !== req.params.username) {
  ...
  try {
    ...
    const profile = await prisma.studentProfile.findUnique({ where: { username: targetUsername } });
    if (!profile) {
    ...
    // Enrichment for admin view or specific student profile check
    // This allows the admin student view to see full academic context
    ...
    if (token && req.params.username) {
        ...
        try {
            const [gradesRes, attendanceRes] = await Promise.all([
                axios.get(`${GATEWAY_URL}/academics/grades?studentId=${targetUsername}`, { headers: { Authorization: token } }).catch(() => ({ data: null })),
                axios.get(`${GATEWAY_URL}/academics/attendance?studentId=${targetUsername}`, { headers: { Authorization: token } }).catch(() => ({ data: null }))
            ]);
...
export const adminUpdateStudentProfile = async (req: AuthenticatedRequest, res: Response) => {
    const user = req.user;
    ...
    try {
        ...
        console.error(`[ERROR] adminUpdateStudentProfile failed for ${username}:`, e.message || e);
        return res.status(500).json({ code: ErrorCode.INTERNAL_SERVER_ERROR, message: 'Failed to update student profile', details: e.message });
    }
};
...
export const searchStudents = async (req: AuthenticatedRequest, res: Response) => {
    ...
    if (!user || user.role === UserRole.STUDENT) {
        return res.status(403).json({ code: ErrorCode.AUTH_FORBIDDEN, message: 'Students cannot search other students' });
    }
...
export const createFacultyProfile = async (req: AuthenticatedRequest, res: Response) => {
    ...
    try {
        ...
        if (e.code === 'P2002') {
             return res.status(409).json({ code: ErrorCode.RESOURCE_ALREADY_EXISTS, message: 'This faculty profile already exists.' });
        }
        return res.status(500).json({ code: ErrorCode.INTERNAL_SERVER_ERROR, message: 'Could not create profile. Please try again.' });
    }
}
export const updateStudentPresence = async (req: AuthenticatedRequest, res: Response) => {
    ...
    if (!user || (!allowedRoles.includes(user.role as UserRole) && user.username !== username)) {
        return res.status(403).json({ code: ErrorCode.AUTH_FORBIDDEN, message: 'Access denied' });
    }
...
export const publishBanner = async (req: AuthenticatedRequest, res: Response) => {
    ...
    try {
        ...
        return res.status(500).json({ success: false, message: 'Failed to update banner status' });
    }
};
