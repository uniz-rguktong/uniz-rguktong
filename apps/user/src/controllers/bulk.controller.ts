...
export const getStudentsTemplate = async (req: AuthenticatedRequest, res: Response) => {
    // Columns: Student ID, Name, Email, Gender (M/F), Branch, Year, Section, Phone
    const headers = [
        ["Student ID", "Name", "Email", "Gender", "Branch", "Year", "Section", "Phone"],
        ["O210000", "ExampleStudent", "o210000@rguktong.ac.in", "Male", "CSE", "E1", "A", "9876543210"]
    ];
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            try {
                await prisma.studentProfile.upsert({
                    where: { username: id },
                    update: { 
                        name, email, gender, branch, year, section, phone, 
                        updatedAt: new Date()
                    },
                    create: {
                        id, username: id, name, email, gender, branch, year, section, phone
                    }
                });
                successCount++;
